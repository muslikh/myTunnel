...
//    }
     ...
     * @throws ConfigException
     */
    public function addFirewallPort($server, $name,$label, $remoteadress, $port,$to_port): array
    {
        $client = $this->getMikrotik($server);
        $addPort = (new Query('/ip/firewall/nat/add'))
            ->equal('chain', 'dstnat')
            ->equal('chain', 'dstnat')
            ->equal('action', 'dst-nat')
            ->equal('to-addresses', $remoteadress)
            ->equal('to-ports', $port)
            ->equal('protocol', 'tcp')
            ->equal('dst-port', $to_port)
            ->equal('comment', strtoupper($name.'-NAT-'.$label))
            ->equal('disabled', 'no');

            // dd($addPort);
        return $client->qr($addPort);
    }
     ...
     * @throws ConfigException
     */
    public function deletePort($server, $port): void
    {
        $client = $this->getMikrotik($server);
        // ==============port================
        $updatePortWinbox = new Query('/ip/firewall/nat/print');
        $updatePortWinbox->where('dst-port', $port);
        $ports = $client->query($updatePortWinbox)->read();
     ...
     * @throws ConfigException
     */
    public function cekPort($server, $port)
    {
        $client = $this->getMikrotik($server);
        // ==============port================
        $cekPort = new Query('/ip/firewall/nat/print');
        $cekPort->where('dst-port', $port);
        $ports = $client->query($cekPort)->read();
