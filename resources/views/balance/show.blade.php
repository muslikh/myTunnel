<x-app-layout>
    <x-slot name="header">
        ...
        <div class="flex justify-end">
            <Link onclick="printContent()"
                class="px-2 py-1.5 bg-red-500 text-indigo-100 font-semibold hover:bg-red-700 hover:text-white rounded-md">
            Cetak</Link>
    ...
    <div class="py-6">
        <div class="mx-auto max-w-screen-lg">
            {{-- style="{{ $detail->status === 'PAID' ? 'background-image: url("asset("img/lunas.png")")' : ($detail->status === 'UNPAID' ? 'background-image: url("asset("img/lunas.png")")' : 'background-image: url("asset("img/lunas.png")")') }}"> --}}
            <div id="report"
                class="rounded-lg border border bg-white p-4 shadow-lg dark:border-gray-200 dark:bg-gray-50 md:p-6 xl:p-9"
                style="{{ $detail->status === 'PAID' ? 'background-image: url("asset("img/lunas.png")")' : ($detail->status === 'UNPAID' ? 'background-image: url("asset("img/lunas.png")")' : 'background-image: url("asset("img/lunas.png")")') }}">
                        ...
                        @foreach ($detail->order_items as $item)
                            <div class="w-full items-center justify-between md:flex">
                                <div class="mb-3 md:mb-0">
                                    <a href="#"
                                        class="inline-block font-medium text-black hover:text-blue dark:text-gray-700">
                                        {{ $item->name }}
                                    </a>
                                    <p class="flex text-sm font-medium">
                                        ...
                                        <span class="mr-5"> Status:
                                            <span
                                                class="min-w-[50px] text-center font-semibold {{ $detail->status === 'PAID' ? 'text-green-500' : ($detail->status === 'UNPAID' ? 'text-red-500' : 'text-yellow-500') }}">
                                                {{ $detail->status }}
                                            </span>
                                        </span>
                                    </p>
                            ...
                            <x-splade-lazy>
                                ...
                                <div>
                                    @if (isset($detail) && property_exists($detail, 'qr_url') && $detail->qr_url)
                                        ...
                                        @else
                                            <img class="w-32" src="{{ $detail->qr_url }}"
                    ...
                    <div class="w-full px-4 xl:w-6/12">
                        <div class="mr-10 text-right md:ml-auto">
                            <div class="ml-auto sm:w-1/2">
                                ...
                                <p
                                    class="mb-4 mt-2 flex justify-between border-t border-b pt-6 font-medium text-black dark:border-gray-200 dark:text-gray-700">
                            ...
                            {{-- <div class="mt-10 flex flex-col justify-end gap-4 sm:flex-row">
                                <Link href="{{ route('dashboard') }}"
                                    class="flex items-center justify-center rounded bg-blue-500 py-2.5 px-8 text-center font-medium text-blue-100 hover:bg-opacity-90">
                                Back
                                </Link>
                            </div> --}}
                        </div>
